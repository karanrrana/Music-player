...
class MusicPlayer:
    ...
    def basic_setup(self):
        # Heading
        Label(self.window, text="Music Player", font=("Arial", 20,
              "bold", "underline"), bg="#a4a5b9", fg="gold").place(x=320, y=25)
        ...
        self.my_list_song = Listbox(frame, bg="#404040", fg="#ffbf50", width=120, height=10, font=(
            "Arial", 8, "bold"), relief=SUNKEN, borderwidth=20, yscrollcommand=v_scroll.set)
    ...
    def instructional_btn_setup(self):  # Some button initialization
        song_add = Button(self.window, text="Add Song", font=("Helvetica", 15, "bold", "italic"), bg="#262626", fg="#ff1414",
                          activebackground="#262626", activeforeground="#ff1414", width=12, height=1, relief=RAISED, borderwidth=5, command=self.add_song)
        song_add.place(x=480, y=352)

        delete_song = Button(self.window, text="Delete Selected Song", font=("Helvetica", 14, "bold", "italic"), bg="#262626", fg="#ff1414",
                             activebackground="#262626", activeforeground="#ff1414", width=27, relief=RAISED, borderwidth=5, command=self.delete_selected_song)
        delete_song.place(x=480, y=437)

        song_counter = Button(self.window, text="Song Counter", width=13, font=("Helvetica", 15, "bold", "italic"), activebackground="#262626",
                              activeforeground="#ff1414", bg="#262626", fg="#ff1414", relief=RAISED, borderwidth=5, command=self.song_counter)
    ...
    def add_song(self):  # Adding songs
        add_multiple_song = filedialog.askopenfilenames(
            title="Select one or multiple song", filetypes=(("MP3 files", "*mp3"), ("WAV files", "*.wav")))
    ...
    def song_counter(self):  # Total song present in the list
        messagebox.showinfo(
            "Song Counter", "Total song in the list: " + str(self.my_list_song.size()))

    def song_duration(self):  # Make Song_Duration Label
        self.song_duration_bar = Label(self.window, text="Song Duration", font=(
            "Arial", 17, "bold", "underline"), bg="#262626", fg="#FFFFFF", activebackground="#262626", activeforeground="#ff1414", width=25, height=1, relief=RAISED, borderwidth=5)
        self.song_duration_bar.place(x=260, y=285)
    ...
    def play_song(self, e=None):  # Play a song
        try:
            ...
            self.song_length = time.strftime(
                "%H:%M:%S", time.gmtime(song_type.info.length))
        ...
        try:
            ...
            if self.song_length == converted_time and self.repeat_counter == 1:
                ...
                self.song_duration_bar.config(
                    text="Time is: "+str(converted_time)+" of "+str(self.song_length))
                # Recursive function call after 1 sec = 1000ms
    ...
    def muter(self):  # Mute set-up
        self.mute_scale = Scale(self.window, from_=1, to=0, orient=HORIZONTAL, bg="#9f9fff", command=self.get_mute,
        ...
        mute_indicator = Label(self.window, text="Mute", font=(
            "Arial", 10, "bold"), fg="blue", bg="#9f9fff")
    ...
    def repeat_controller(self):  # Repeat Set-up
        self.loop_bar = Scale(self.window, from_=1, to=0, orient=HORIZONTAL, bg="#9f9fff", command=self.repeat_maintain,
        ...
        loop_bar_indicator = Label(self.window, text="Off    Repeat    On", font=(
            "Arial", 10, "bold"), fg="blue", bg="#9f9fff")
    ...
    def shuffle_controller(self):  # Shuffle set-up
        self.shuffle_bar = Scale(self.window, from_=1, to=0, orient=HORIZONTAL, bg="#9f9fff", command=self.shuffle_maintain,
                                 activebackground="red", font=("Arial", 15, "bold"), length=140, relief=RIDGE, bd=3)
        self.shuffle_bar.place(x=305, y=430)

        self.shuffle_bar.set(self.shuffle_status)

        shuffle_bar_indicator = Label(self.window, text="  Off    Shuffle    On", font=(
            "Arial", 10, "bold"), fg="blue", bg="#9f9fff")
...
if __name__ == '__main__':
    ...
    window.config(bg="#a4a5b9")  # 141414
    background_image = PhotoImage(file="img1.png")

    background = Label(window, image=background_image, bd=0)
    background.pack()

    backward_image_take = ImageTk.PhotoImage(
        Image.open('Pictures/backword22.png'))
    backward_btn_img = Button(window, image=backward_image_take,
                              bg="#8575f0", activebackground="#323232", relief=RAISED, bd=3)
    backward_btn_img.place(x=25, y=350)

    play_image_take = ImageTk.PhotoImage(Image.open(
        'Pictures/play22.png').resize((40, 40), Image.ANTIALIAS))
    play_btn_img = Button(window, image=play_image_take, bg="#8575f0",
                          activebackground="#323232", relief=RAISED, bd=3)
    play_btn_img.place(x=115, y=350)  # 323232
    play_btn_img.place(x=115, y=350)

    pause_image_take = ImageTk.PhotoImage(Image.open('Pictures/pause22.png'))
    pause_btn_img = Button(window, image=pause_image_take, bg="#8575f0",
                           activebackground="#323232", relief=RAISED, bd=3)
    pause_btn_img.place(x=210, y=350)

    stop_image_take = ImageTk.PhotoImage(Image.open(
        'Pictures/stop22.png').resize((40, 40), Image.ANTIALIAS))
    stop_btn_img = Button(window, image=stop_image_take, bg="#8575f0",
                          activebackground="#323232", relief=RAISED, bd=3)
    stop_btn_img.place(x=305, y=350)

    forward_image_take = ImageTk.PhotoImage(
        Image.open('Pictures/forward22.png'))
    forward_btn_img = Button(window, image=forward_image_take,
                             bg="#8575f0", activebackground="#323232", relief=RAISED, bd=3)
    forward_btn_img.place(x=400, y=350)

    MusicPlayer(window, backward_btn_img, play_btn_img,
                pause_btn_img, stop_btn_img, forward_btn_img)
